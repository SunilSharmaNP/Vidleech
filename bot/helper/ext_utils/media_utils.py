...
# ============================================================
# 🔧 VIDEO TOOLKIT EXTENSIONS (For "Video Tools" Menu)
# ============================================================
from bot import subprocess_lock
import os

async def run_ffmpeg_tool(cmd: list):
    """
    Run ffmpeg tool asynchronously with safety, logging and global lock.
    """
    async with subprocess_lock:
        ...
        if code == 0:
            ...
            return True
        LOGGER.error(f"❌ FFmpeg process failed: {stderr.decode().strip()}")
        return False


async def compress_video(input_path: str, output_path: str, crf: int = 28):
    """
    Compress video file to reduce size (CRF 18=high, 28=low quality).
    """
    cmd = [FFMPEG_NAME, '-hide_banner', '-i', input_path, '-vcodec', 'libx264',
           '-preset', 'medium', '-crf', str(crf), '-c:a', 'aac', '-b:a', '128k',
           '-movflags', '+faststart', output_path, '-y']
    return await run_ffmpeg_tool(cmd)


async def mute_video(input_path: str, output_path: str):
    """
    Removes audio stream from the video.
    """
    cmd = [FFMPEG_NAME, '-hide_banner', '-i', input_path, '-an', '-c:v', 'copy', output_path, '-y']
    return await run_ffmpeg_tool(cmd)
...
async def add_audio(video_path: str, audio_path: str, output_path: str):
    ...
    """
    cmd = [FFMPEG_NAME, '-hide_banner', '-i', video_path, '-i', audio_path, '-c:v', 'copy',
           '-map', '0:v:0', '-map', '1:a:0', '-shortest', '-c:a', 'aac', output_path, '-y']
    return await run_ffmpeg_tool(cmd)


async def trim_video(input_path: str, start: str, end: str, output_path: str):
    """
    Cuts or trims video segment between HH:MM:SS times.
    Example: start="00:00:10", end="00:01:00"
    """
    cmd = [FFMPEG_NAME, '-hide_banner', '-ss', start, '-to', end, '-i', input_path,
           '-c:v', 'copy', '-c:a', 'copy', output_path, '-y']
    return await run_ffmpeg_tool(cmd)


async def merge_videos(video_list: list[str], output_path: str):
    """
    Merges multiple videos of same codec using concat demuxer.
    """
    temp_file = 'merge_list.txt'
    async with open(temp_file, 'w') as f:
        for v in video_list:
            await f.write(f"file '{v}'\n")
    cmd = [FFMPEG_NAME, '-hide_banner', '-f', 'concat', '-safe', '0', '-i', temp_file, '-c', 'copy', output_path, '-y']
    success = await run_ffmpeg_tool(cmd)
    if os.path.exists(temp_file):
        os.remove(temp_file)
    return success


async def watermark_video(input_path: str, watermark_path: str, output_path: str, position: str = "top-right"):
    """
    Adds watermark overlay image at desired position (top-left, top-right, bottom-left, bottom-right).
    """
    positions = {
        "top-left": "10:10",
        "top-right": "W-w-10:10",
    ...
    cmd = [FFMPEG_NAME, '-hide_banner', '-i', input_path, '-i', watermark_path,
           '-filter_complex', f'overlay={pos}', '-codec:a', 'copy', output_path, '-y']
    return await run_ffmpeg_tool(cmd)


async def convert_format(input_path: str, output_path: str):
    """
    Converts video file to target format (based on extension of output_path).
    Example: convert_format("video.mkv", "video.mp4")
    """
    cmd = [FFMPEG_NAME, '-hide_banner', '-i', input_path, '-c:v', 'copy', '-c:a', 'copy', output_path, '-y']
    return await run_ffmpeg_tool(cmd)
