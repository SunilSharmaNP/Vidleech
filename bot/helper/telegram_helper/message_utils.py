...
def style_text(text: str) -> str:
    """
    Apply consistent visual formatting across bot messages.
    """
    text = text.replace("USER SETTINGS", "⚙️ <b>USER SETTINGS</b>")
    text = text.replace("VIDEO TOOLS", "🎞️ <b>VIDEO TOOLS</b>")
    text = text.replace("UNIVERSAL SETTINGS", "🪄 <b>UNIVERSAL SETTINGS</b>")
    text = text.replace("MIRROR SETTINGS", "🪞 <b>MIRROR SETTINGS</b>")
    text = text.replace("LEECH SETTINGS", "📦 <b>LEECH SETTINGS</b>")
    return text
...
# =========================================================
# MESSAGE DECORATOR (handles floodwaits and clean errors)
# =========================================================
...
def handle_message(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            LOGGER.info(f"⏳ Waiting {f.value}s before retry ({func_name})")
...
# =========================================================
# CORE SEND / EDIT FUNCTIONS
# =========================================================

async def sendingMessage(text: str, message: Message, photo=None, reply_markup: InlineKeyboardMarkup = None):
    """
    Smart message sender — automatically picks image/text mode depending on config and file existence.
    Adds header emojis & trims message safely.
    """
    formatted_text = style_text(limit.text(text.strip()))

    if config_dict.get('ENABLE_IMAGE_MODE', False) and photo:
        try:
            return await sendPhoto(formatted_text, message, photo, reply_markup)
        except Exception as e:
            LOGGER.warning(f"Fallback to text mode due to error: {e}")
            return await sendMessage(formatted_text, message, reply_markup)
    else:
        return await sendMessage(formatted_text, message, reply_markup)
...
async def sendMessage(text: str, message: Message, reply_markup: InlineKeyboardMarkup = None, block=True):
    formatted = style_text(limit.text(text.strip()))
...
async def sendMedia(caption: str, chat_id: int, reply_to: Message, reply_markup: InlineKeyboardMarkup = None):
    formatted = style_text(limit.caption(caption))
    return await reply_to.copy(chat_id, formatted, reply_markup=reply_markup, disable_notification=True)
...
async def sendCustom(text: str, chat_id: str | int, reply_markup: InlineKeyboardMarkup = None, nolog=False):
    formatted = style_text(limit.text(text.strip()))
    return await bot.send_message(chat_id, formatted, reply_markup=reply_markup, disable_notification=True)


@handle_message
async def editCustom(text: str, chat_id: int, message_id: int, reply_markup=None):
    formatted = style_text(limit.text(text.strip()))
    return await bot.edit_message_text(chat_id, message_id, formatted, reply_markup=reply_markup, disable_web_page_preview=True)


@handle_message
async def editMessage(text: str, message: Message, reply_markup: InlineKeyboardMarkup = None, block=True):
    formatted = style_text(limit.text(text.strip()))
    return await message.edit_text(formatted, reply_markup=reply_markup, disable_web_page_preview=True)
...
async def sendPhoto(caption: str, message: Message, photo, reply_markup: InlineKeyboardMarkup = None):
    formatted = style_text(limit.caption(caption.strip()))
    if "USER SETTINGS" in formatted or "VIDEO TOOLS" in formatted:
...
async def editPhoto(caption: str, message: Message, photo, reply_markup: InlineKeyboardMarkup = None):
    formatted = style_text(limit.caption(caption.strip()))
    return await message.edit_media(InputMediaPhoto(photo, formatted), reply_markup)
...
@handle_message
async def alert(query, text: str, stime: int = 3):
    """
    Display alert-style popup for quick user feedback.
    Example: await alert(query, "⚠️ This feature is under development!")
    """
    try:
...
# =========================================================
# STATUS HANDLING (unchanged core)
# =========================================================
