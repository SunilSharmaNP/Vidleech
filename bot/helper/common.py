    ...
    async def generateSampleVideo(self, dl_path, gid):
        ...
        async with cpu_eater_lock:
            ...
            if await aiopath.isfile(dl_path):
                ...
                return dl_path
# ============================================================
# VIDEO TOOLS (For /UserSettings → Video Tools Menu)
# ============================================================
import os

async def run_ffmpeg_command(cmd: list):
    """
    Executes ffmpeg command asynchronously with logging and error handling.
    """
...
async def compress_video(input_path: str, output_path: str):
    """
    Compress video using ffmpeg (reduces bitrate, keeps quality reasonable).
    """
    cmd = [FFMPEG_NAME, '-i', input_path, '-b:v', '1M', '-b:a', '128k', '-preset', 'medium', '-c:v', 'libx264', '-c:a', 'aac', output_path, '-y']
    return await run_ffmpeg_command(cmd)


async def convert_format(input_path: str, output_path: str):
    """
    Converts video to another format (by output extension, e.g. .mp4, .mkv).
    """
    cmd = [FFMPEG_NAME, '-i', input_path, '-c:v', 'copy', '-c:a', 'copy', output_path, '-y']
    return await run_ffmpeg_command(cmd)
...
async def mute_video(input_path: str, output_path: str):
    ...
    """
    cmd = [FFMPEG_NAME, '-i', input_path, '-an', '-c:v', 'copy', output_path, '-y']
    return await run_ffmpeg_command(cmd)


async def add_audio_to_video(video_path: str, audio_path: str, output_path: str):
    """
    Adds background audio/music to a video file.
    """
    cmd = [FFMPEG_NAME, '-i', video_path, '-i', audio_path, '-c:v', 'copy', '-c:a', 'aac', '-map', '0:v:0', '-map', '1:a:0', '-shortest', output_path, '-y']
    return await run_ffmpeg_command(cmd)


async def trim_video(input_path: str, start_time: str, end_time: str, output_path: str):
    """
    Trims/cuts video between given start and end times (HH:MM:SS format).
    """
    cmd = [FFMPEG_NAME, '-i', input_path, '-ss', start_time, '-to', end_time, '-c:v', 'copy', '-c:a', 'copy', output_path, '-y']
    return await run_ffmpeg_command(cmd)


async def merge_videos(video_list: list, output_path: str):
    """
    Merges multiple videos together. Requires all videos to have same codec.
    """
    list_file = 'videos_to_merge.txt'
    async with open(list_file, 'w') as f:
        for v in video_list:
            await f.write(f"file '{v}'\n")

    cmd = [FFMPEG_NAME, '-f', 'concat', '-safe', '0', '-i', list_file, '-c', 'copy', output_path, '-y']
...
async def add_watermark(input_path: str, watermark_path: str, output_path: str):
    """
    Adds watermark (image/logo) overlay to top-right of video.
    """
    cmd = [FFMPEG_NAME, '-i', input_path, '-i', watermark_path, '-filter_complex', 'overlay=W-w-10:10', '-codec:a', 'copy', output_path, '-y']
    return await run_ffmpeg_command(cmd)
