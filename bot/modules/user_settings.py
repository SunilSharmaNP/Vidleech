...
async def get_user_settings(from_user, data: str, uset_data: str):
    ...
    # ----------------- Universal Settings submenu -----------------
        ...
        # YT-DLP Options status
        yt_opt_status = 'Not Exists ❌' if not (user_dict.get('yt_opt') or config_dict.get('YT_DLP_OPTIONS')) else 'Exists ✅'
        
        # Session String status
        session_status = 'Not Exists ❌' if not user_dict.get('session_string') else 'Exists ✅'
        
        # Bot PM status
        bot_pm_enabled = not user_dict.get('enable_pm', False)
        bot_pm_status = 'Force Enabled' if bot_pm_enabled else 'Disabled'
        
        # MediaInfo status
        media_info = user_dict.get('media_info', config_dict.get('MEDIA_INFO', False))
        media_info_status = 'Enabled' if media_info else 'Disabled'
        
        # Save Mode status
        dump_ch = user_dict.get('dump_ch')
        save_mode_status = 'Save As BotPM' if not dump_ch else f'Save As Dump'
        
        # Daily Tasks status
        daily_limit = user_dict.get('daily_limit')
        daily_tasks = '∞ / ∞ per day' if not daily_limit else f'{daily_limit} / {config_dict.get("DAILY_LIMIT_SIZE", "∞")} per day'
        
        # Last Used
        last_used_time = user_dict.get('last_used_time')
        if last_used_time:
            elapsed = time() - last_used_time
            last_used = get_readable_time(elapsed) + ' ago'
        else:
            last_used = 'Never'
        
        # Build buttons
        buttons.button_data('YT-DLP OPTIONS', f'userset {user_id} setdata yt_opt')
        buttons.button_data('USER SESSION', f'userset {user_id} setdata session_string')
        buttons.button_data('DISABLE BOT PM', f'userset {user_id} enable_pm')
        buttons.button_data('ENABLE MEDIAINFO', f'userset {user_id} media_info')
        buttons.button_data('SAVE AS DUMP', f'userset {user_id} setdata dump_ch')
        buttons.button_data('Back', f'userset {user_id} back')
        buttons.button_data('Close', f'userset {user_id} close')

        # Compose text summary
        text = (
            f'<b>☣️ Universal Settings : {from_user.mention}</b>\n\n'
            f'<b>📺 YT-DLP Options :</b> {yt_opt_status}\n'
            f'<b>📅 Daily Tasks :</b> {daily_tasks}\n'
            f'<b>🕒 Last Used :</b> {last_used}\n'
            f'<b>📊 MediaInfo :</b> {media_info_status}\n'
            f'<b>💬 Bot PM :</b> {bot_pm_status}\n'
            f'<b>💾 Save Mode :</b> {save_mode_status}'
        )
        image = config_dict.get('IMAGE_USETIINGS')
        return text, image, buttons.build_menu(2)
    ...
    # ----------------- Mirror Settings submenu -----------------
        ...
        # Daily Mirror status
        daily_mirror = '∞ / ∞ per day'
        
        # Prefix status
        prefix_status = 'Not Exists ❌' if not user_dict.get('prename') else f'{user_dict.get("prename")} ✅'
        
        # Suffix status
        suffix_status = 'Not Exists ❌' if not user_dict.get('sufname') else f'{user_dict.get("sufname")} ✅'
        
        # Remname status
        remname_status = 'Not Exists ❌' if not user_dict.get('remname') else 'Exists ✅'
        
        # DDL Server status - Placeholder (feature not yet implemented)
        # In future, this will read from a dedicated DDL servers configuration
        ddl_count = 0  # Placeholder
        
        # RClone status
        rclone_path = ospath.join('rclone', f'{user_id}.conf')
        rclone_status = 'Not Exists ❌'
        if await aiopath.exists(rclone_path):
            rclone_status = 'Exists ✅'
        
        # User TD status - check gdrive_id
        user_td_status = 'Force Disabled'
        if user_dict.get('gdrive_id') or config_dict.get('GDRIVE_ID'):
            user_td_status = 'Enabled'
        
        # TD Info - Placeholder (feature will count team drives when fully implemented)
        # For now showing 0 until team drive management is complete
        td_info = 0  # Placeholder
        
        # Build buttons
        buttons.button_data('RCLONE', f'userset {user_id} rctool')
        buttons.button_data('MIRROR PREFIX', f'userset {user_id} setdata prename')
        buttons.button_data('MIRROR SUFFIX', f'userset {user_id} setdata sufname')
        buttons.button_data('MIRROR REMNAME', f'userset {user_id} setdata remname')
        buttons.button_data('DDL SERVERS', f'userset {user_id} ddl_servers')
        buttons.button_data('USER TD', f'userset {user_id} gdtool')
        buttons.button_data('Back', f'userset {user_id} back')
        buttons.button_data('Close', f'userset {user_id} close')

        text = (
            f'<b>☣️ Mirror/Clone Settings : {from_user.mention}</b>\n\n'
            f'<b>📅 Daily Mirror :</b> {daily_mirror}\n'
            f'<b>➕ Prefix :</b> {prefix_status}\n'
            f'<b>➖ Suffix :</b> {suffix_status}\n'
            f'<b>🌈 Renmame :</b> {remname_status}\n'
            f'<b>🌐 DDL Server(s) :</b> {ddl_count}\n'
            f'<b>🗂 RClone :</b> {rclone_status}\n'
            f'<b>📁 User TD :</b> {user_td_status}\n'
            f'<b>📂 TD Info:</b> {td_info}'
        )
    ...
    # ----------------- Leech Settings submenu -----------------
        ...
        # Leech Type
        leech_type = 'DOCUMENT 📁' if user_dict.get('as_doc') else 'MEDIA 📺'
        
        # Thumbnail status
        thumbpath = ospath.join('thumbnails', f'{user_id}.jpg')
        thumb_status = 'Not Exists ❌' if not await aiopath.exists(thumbpath) else 'Exists ✅'
        
        # Split Size
        split_size_value = user_dict.get('split_size', config_dict.get('LEECH_SPLIT_SIZE', 2097152000))
        split_size = f'{get_readable_file_size(split_size_value)} (Default)' if not user_dict.get('split_size') else get_readable_file_size(split_size_value)
        
        # Equal Splits
        equal_splits = 'Disabled' if not user_dict.get('equal_splits') else 'Enabled'
        
        # Media Group
        media_group = 'Disabled' if not user_dict.get('media_group') else 'Enabled'
        
        # Caption status
        caption_status = 'Not Exists ❌' if not user_dict.get('captions') else 'Exists ✅'
        ...
        # Metadata status
        metadata_status = 'Not Exists ❌' if not user_dict.get('metadata') else 'Exists ✅'
        
        # Attachment status - check for attachment_url in user_dict
        attachment_url = user_dict.get('attachment_url', config_dict.get('ATTACHMENT_URL', ''))
        attachment_status = 'Not Exists ❌' if not attachment_url else 'Exists ✅'
        
        # Dump status
        dump_status = 'Not Exists ❌' if not user_dict.get('dump_ch') else 'Exists ✅'
        ...
        buttons.button_data('LEECH SPLITS', f'userset {user_id} split_size_menu')
        buttons.button_data('LEECH CAPTION', f'userset {user_id} capmode')
        buttons.button_data('LEECH PREFIX', f'userset {user_id} setdata prename')
        buttons.button_data('LEECH SUFFIX', f'userset {user_id} setdata sufname')
        buttons.button_data('LEECH REMNAME', f'userset {user_id} setdata remname')
        buttons.button_data('LEECH DUMP', f'userset {user_id} setdata dump_ch')
        buttons.button_data('METADATA TEXT', f'userset {user_id} setdata metadata')
        buttons.button_data('ATTACHMENT URL', f'userset {user_id} attachment')
        buttons.button_data('Back', f'userset {user_id} back')
        buttons.button_data('Close', f'userset {user_id} close')

        text = (
            f'<b>☣️ Leech Settings for {from_user.mention}</b>\n\n'
            f'<b>📅 Daily Leech :</b> {daily_leech}\n'
            f'<b>🎬 Leech Type :</b> {leech_type}\n'
            f'<b>📸 Thumbnail :</b> {thumb_status}\n'
            f'<b>✂️ Split Size :</b> {split_size}\n'
            f'<b>🟰 Equal Splits :</b> {equal_splits}\n'
            f'<b>📦 Media Group :</b> {media_group}\n'
            f'<b>💬 Caption :</b> {caption_status}\n'
            f'<b>➕ Prefix :</b> {prefix_status}\n'
            f'<b>➖ Suffix :</b> {suffix_status}\n'
            f'<b>📊 Metadata :</b> {metadata_status}\n'
            f'<b>📎 Attachment :</b> {attachment_status}\n'
            f'<b>🗑 Dump :</b> {dump_status}\n'
            f'<b>🌈 Renmame :</b> {remname_status}'
        )
        image = config_dict.get('IMAGE_USETIINGS')
        return text, image, buttons.build_menu(2)
    ...
    elif data == 'ddl_servers':
        # DDL Servers placeholder panel
        buttons.button_data('Back', f'userset {user_id} mirror')
        buttons.button_data('Close', f'userset {user_id} close')
        
        text = (
            '<b>DDL SERVERS</b>\n\n'
            '<i>DDL Server management feature is currently under development.</i>\n\n'
            'This feature will allow you to configure direct download link servers for your uploads.'
        )
        image = config_dict.get('IMAGE_USETIINGS')
        return text, image, buttons.build_menu(2)

    elif data == 'split_size_menu':
        # Split Size Settings submenu
        split_size_value = user_dict.get('split_size', config_dict.get('LEECH_SPLIT_SIZE', 2097152000))
        split_size_display = get_readable_file_size(split_size_value)
        equal_splits = user_dict.get('equal_splits', False)
        
        buttons.button_data('Set Split Size', f'userset {user_id} setdata split_size')
        buttons.button_data('Toggle Equal Splits', f'userset {user_id} equal_splits')
        buttons.button_data('Back', f'userset {user_id} leech')
        buttons.button_data('Close', f'userset {user_id} close')
        
        text = (
            f'<b>SPLIT SIZE SETTINGS</b>\n\n'
            f'<b>Current Split Size:</b> {split_size_display}\n'
            f'<b>Equal Splits:</b> {"Enabled ✅" if equal_splits else "Disabled ❌"}\n\n'
            f'<i>Split size determines how large files are split for leech.</i>'
        )
        image = config_dict.get('IMAGE_USETIINGS')
        return text, image, buttons.build_menu(2)
        ...
        if uset_data in {'thumb', 'rclone_config', 'token_pickle'}:
            ...
            uset_dict = {
                ...
                ('attachment_url', 'Attachment URL',
                 'Send attachment URL to set.\n\n<i>Timeout: 60s.</i>',
                 config_dict.get('IMAGE_USETIINGS'), 'leech'),
                'split_size':
                ('split_size', 'Split Size',
                 f'Send split size in bytes (current: {get_readable_file_size(user_dict.get("split_size", config_dict.get("LEECH_SPLIT_SIZE", 2097152000)))})\n\n<i>Timeout: 60s.</i>',
                 config_dict.get('IMAGE_USETIINGS'), 'split_size_menu'),
...
async def set_user_settings(_, message: Message, query: CallbackQuery,
    ...
    if (key == 'dump_ch' and value.isdigit() or value.startswith('-100')):
        ...
        # Handle split size - convert to integer bytes
        try:
            value = int(value)
            if value < 1048576:  # Minimum 1MB
                value = 1048576
            elif value > 4294967296:  # Maximum 4GB
                value = 4294967296
        except ValueError:
            await sendMessage('Invalid split size! Please send a number in bytes.', message)
            return
...
async def edit_user_settings(client: Client, query: CallbackQuery):
    ...
    match data[2]:
        ...
        case 'rem_prename' | 'rem_sufname' | 'rem_dump_ch' | 'rem_remname' | 'rem_metadata' | 'rem_session_string' | 'rem_yt_opt' | 'rem_index_url' \
            | 'rem_gdrive_id' | 'rem_captions' | 'rem_excluded_extensions' | 'rem_rclone_path' | 'rem_attachment_url' | 'rem_split_size' as value:
            qdata = uset_data = ''
            match value:
                case 'rem_dump_ch':
        ...
        case 'enable_pm' | 'enable_ss' | 'as_doc' | 'media_group' | 'media_info' | 'fnamecap' | 'stop_duplicate' | 'use_sa' | 'equal_splits' as value:
            qdata = uset_data = ''
            ...
            if value == 'fnamecap':
                ...
                qdata = 'gdtool'
            elif value == 'media_info':
                qdata = 'universal'
            elif value == 'enable_pm':
                qdata = 'universal'
            await gather(query.answer(),
                         update_user_settings(query, qdata, uset_data))

        case 'capmode' | 'gdtool' | 'rctool' | 'universal' | 'mirror' | 'leech' as value:
        ...
        # ----------------- New handlers for additional options -----------------
        case 'ddl_servers':
            # Placeholder - DDL servers feature not yet implemented
            await gather(query.answer('DDL Servers: Feature under development!', True), 
                        update_user_settings(query, 'mirror'))
        
        case 'split_size_menu':
            # Route to Split Size settings menu
            await gather(query.answer(), update_user_settings(query, 'split_size_menu'))
        
        case 'attachment':
            # Route to Attachment URL configuration
            await gather(query.answer(), update_user_settings(query, 'setdata', 'attachment_url'))
