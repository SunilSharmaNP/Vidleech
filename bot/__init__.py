    ...
    if not DATABASE_URL.startswith('mongodb'):
        try:
            DATABASE_URL = b64decode(
                resub('ini|adalah|pesan|yang|sangat|rahasia', '',
                      DATABASE_URL)).decode('utf-8')
        except:
            pass
    try:
        ...
        if old_config is None:
            db.settings.deployConfig.replace_one({'_id': bot_id},
                                                 current_config,
                                                 upsert=True)
        else:
            del old_config['_id']
...
# ✅ Auto-select writable path
if os.access('/usr/src/app', os.W_OK):
    base_dir = '/usr/src/app'
else:
    base_dir = os.getcwd()  # usually /home/runner/workspace on Replit

# ✅ Set DOWNLOAD_DIR safely
DOWNLOAD_DIR = os.environ.get('DOWNLOAD_DIR', os.path.join(base_dir, 'downloads/'))
if not DOWNLOAD_DIR.endswith('/'):
    DOWNLOAD_DIR += '/'

# ✅ Create folder safely (no crash on read-only FS)
try:
...
ENABLE_STREAM_LINK = environ.get('ENABLE_STREAM_LINK',
                                 'False').lower() == 'true'
...
INCOMPLETE_TASK_NOTIFIER = environ.get('INCOMPLETE_TASK_NOTIFIER',
                                       'True').lower() == 'true'
INCOMPLETE_AUTO_RESUME = environ.get('INCOMPLETE_AUTO_RESUME',
                                     'True').lower() == 'true'
...
VIDTOOLS_FAST_MODE = environ.get('VIDTOOLS_FAST_MODE',
                                 'False').lower() == 'true'
...
# ============================== LOGS ==================================
ONCOMPLETE_LEECH_LOG = environ.get('ONCOMPLETE_LEECH_LOG',
                                   'True').lower() == 'true'
...
MIRROR_LOG = int(MIRROR_LOG) if MIRROR_LOG.isdigit() or MIRROR_LOG.startswith(
    '-') else MIRROR_LOG

OTHER_LOG = environ.get('OTHER_LOG', '')
OTHER_LOG = int(OTHER_LOG) if OTHER_LOG.isdigit() or OTHER_LOG.startswith(
    '-') else OTHER_LOG

LINK_LOG = environ.get('LINK_LOG', '')
LINK_LOG = int(
    LINK_LOG) if LINK_LOG.isdigit() or LINK_LOG.startswith('-') else LINK_LOG
# ======================================================================
...
TORRENT_DIRECT_LIMIT = float(
    TORRENT_DIRECT_LIMIT) if TORRENT_DIRECT_LIMIT else ''
...
MUTE_CHAT_ID = environ.get('MUTE_CHAT_ID', '')
...
GD_INFO = environ.get('GD_INFO', 'By SSBots')
...
RSS_CHAT = int(
    RSS_CHAT) if RSS_CHAT.isdigit() or RSS_CHAT.startswith('-') else RSS_CHAT
...
PORT = environ.get('PORT')
Popen(f"gunicorn web.wserver:app --bind 0.0.0.0:{PORT} --worker-class gevent",
      shell=True)

try:
    srun([QBIT_NAME, '-d', f'--profile={getcwd()}'], check=True)
except:
    LOGGER.info('qBittorrent already running or failed to start')
...
srun([ARIA_NAME, f'--conf-path={ospath.join(getcwd(), "a2c.conf")}'],
     check=True)
...
if ospath.exists('accounts.zip'):
    ...
    srun('7z x -o. -aoa accounts.zip accounts/*.json && chmod -R 777 accounts',
         shell=True)
...
def get_client():
    return qbClient(host='localhost',
                    port=8090,
...
try:
    ...
    scheduler = AsyncIOScheduler(timezone='UTC', event_loop=bot_loop)
